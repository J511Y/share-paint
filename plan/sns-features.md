...
## Phase 1: 그림 업로드 및 저장 (1주차)

### 1.1 Supabase Storage 설정
- `paintings` 버킷 생성 및 정책 설정 (이미 완료됨: 002_storage_policies.sql)
- 이미지 업로드 유틸리티 함수 구현 (`uploadImage`)

### 1.2 그림 저장 API & UI
- **Single Mode**: `DrawingCanvas`에 '저장/공유' 버튼 추가 및 업로드 모달 구현
- **Battle Mode**: 게임 종료 시 내 그림을 갤러리에 저장하는 옵션 추가
- `POST /api/paintings` 구현

---

## Phase 2: 프로필 페이지 (1.5주차)
...
### 2.2 프로필 편집
- 프로필 수정 모달 (아바타 업로드, 닉네임 변경)
- `PUT /api/users/profile` 구현

### 2.3 유저 갤러리
- 탭 구분: '내 그림', '좋아요한 그림', '대결 기록'
- 그리드 레이아웃으로 그림 목록 표시
...
## Phase 4: 소셜 상호작용 (2.5주차)

### 4.1 좋아요 & 팔로우
- Optimistic UI 적용 (서버 응답 전 UI 즉시 반영)
- `useLike`, `useFollow` 커스텀 훅 구현

### 4.2 댓글 시스템
- 그림 상세 페이지 내 댓글 작성/삭제
- 댓글 목록 실시간 업데이트 (Supabase Realtime 고려)
