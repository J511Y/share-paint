...
export function BattleCanvas({ battleId, className }: BattleCanvasProps) {
  const canvasRef = useRef<CanvasHandle>(null);
  const { updateCanvas } = useBattle(battleId);
  ...
  const handleUndo = useCallback(() => {
    ...
    // Undo 후에도 상태 업데이트 전송 필요
    const dataUrl = canvasRef.current?.getDataUrl();
    if (dataUrl) updateCanvas(dataUrl);
  }, [updateCanvas]);

  const handleRedo = useCallback(() => {
    canvasRef.current?.redo();
    // Redo 후에도 상태 업데이트 전송 필요
    const dataUrl = canvasRef.current?.getDataUrl();
    if (dataUrl) updateCanvas(dataUrl);
  }, [updateCanvas]);
